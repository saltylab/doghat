...
- (void)configureBars {
    ...
    for(int i = 0; i < 12; i++){
        ...
        sampleButton.frame = CGRectMake((i%2)*frame.size.width/2, (i/2)*45 + 100, frame.size.width/2, 45);
        [sampleButton setTitle:titles[i] forState:UIControlStateNormal];
        [sampleButton addTarget:self action:@selector( onTapButton: ) forControlEvents:UIControlEventTouchUpInside];
        sampleButton.tag = i;
...
{
    NSString *string = [NSString stringWithFormat:@"%02ld", (sender.tag+1)];
    NSURL *audioFileURL = [[NSBundle mainBundle] URLForResource:string withExtension:@"mp3"];
    NSError *error;
